...
async function apiGet(resource,params=''){const response=await fetch(`${ARAPONGAS_API}/${resource}${params}`,{headers:{Accept:'application/json'}});if(!response.ok)throw new Error(`API ${response.status}`);return response.json()}
...
window.ArapongasDigitalAPI={
 ...
 productsByBusiness:id=>apiGet('v_public_products',`?select=*&business_id=eq.${qs(id)}&order=featured.desc,name.asc&limit=100`),
 servicesByBusiness:id=>apiGet('v_public_services',`?select=*&business_id=eq.${qs(id)}&order=name.asc&limit=100`),
 locationsByBusiness:id=>apiGet('v_public_locations',`?select=*&business_id=eq.${qs(id)}`),
 hoursByBusiness:id=>apiGet('v_public_hours',`?select=*&business_id=eq.${qs(id)}&order=weekday.asc`),
