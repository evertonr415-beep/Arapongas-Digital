...
window.ArapongasDigitalAPI={
  ...
  productById:(id)=>apiGet('v_public_products',`?select=*&id=eq.${qs(id)}&limit=1`),
  ...
  servicesByBusiness:(businessId)=>apiGet('v_public_services',`?select=*&business_id=eq.${qs(businessId)}&order=name.asc`),
  locationsByBusiness:(businessId)=>apiGet('v_public_locations',`?select=*&business_id=eq.${qs(businessId)}&order=name.asc`),
  hoursByBusiness:(businessId)=>apiGet('v_public_hours',`?select=*&business_id=eq.${qs(businessId)}&order=weekday.asc`),
  plans:()=>apiGet('v_public_plans','?select=*&order=price_monthly.asc,name.asc'),
